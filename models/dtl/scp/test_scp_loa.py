...
def get_https_proxy():
    ...
    print(proxy, proxy[0], proxy[1])
    proxies = {'https': 'https://{0}:{1}'.format(proxy[0],proxy[1])}
    return proxies

def get_page_text(proxies):
    headers={'User-Agent': 'Mozilla/5.0'}
    html = requests.get('https://www.landsofamerica.com/Wayne-County-PA/all-land/', verify=False, headers=headers, proxies=proxies, timeout=2.0)
    return html